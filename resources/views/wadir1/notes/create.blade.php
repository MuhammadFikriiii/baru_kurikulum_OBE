        ...
        <div class="bg-white rounded-lg shadow overflow-hidden p-6">
            <form action="{{ route('wadir1.notes.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="prodi_id" id="prodi_id" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500" required>
                        ...
                        @foreach($prodis as $prodi)
                            <option value="{{ $prodis->id }}">{{ $prodi->nama_prodi }}</option>
                        @endforeach
                    </select>
                    @error('prodi_id')
                ...
                <div class="mb-4">
                    ...
                    <textarea name="note" id="note" rows="6" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="Tulis catatan Anda di sini..." required></textarea>
                    @error('note')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
