...
<div class="container mx-auto px-4">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div class="flex flex-col md:flex-row gap-4 w-full md:w-auto">
            <form method="GET" action="{{ route('admin.profillulusan.index') }}" class="w-full md:w-64">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-green-800 text-white">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-center text-xs font-medium uppercase tracking-wider w-48">Deskripsi</th>
                        <th class="px-4 py-3 text-center text-xs font-medium uppercase tracking-wider w-96">Profesi</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="{{ $index % 2 == 0 ? 'bg-gray-50' : 'bg-white' }} hover:bg-gray-100">
                        ...
                        <td class="px-4 py-4 text-center text-sm">{{ $profillulusan->prodi->nama_prodi ?? '-' }}</td>
                        <td class="px-4 py-4 text-sm whitespace-normal">{{ $profillulusan->deskripsi_pl }}</td>
                        <td class="px-4 py-4 text-sm whitespace-pre-line">{{ $profillulusan->profesi_pl }}</td>
                        <td class="px-4 py-4 text-sm">{{ $profillulusan->unsur_pl }}</td>
                        <td class="px-4 py-4 text-sm text-center">{{ $profillulusan->keterangan_pl }}</td>
                        <td class="px-4 py-4 text-sm">{{ $profillulusan->sumber_pl }}</td>
                        <td class="px-4 py-4">
                            <div class="flex justify-center space-x-2">
                                ...
                                <form action="{{ route('admin.profillulusan.destroy', $profillulusan->id_pl) }}" method="POST">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                                            class="bg-red-600 hover:bg-red-800 text-white p-2 rounded-md"
                                            title="Hapus"
                                            onclick="return confirm('Hapus profil lulusan ini?')">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
