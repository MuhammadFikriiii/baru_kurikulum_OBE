...
<div class="mx-20">
    ...
    <form action="{{ route('admin.bobot.store') }}" method="POST" id="bobotForm">
        ...
        <div id="mkSection" class="mt-6 hidden">
            ...
            <div id="mkList" class="mt-2 border border-black rounded-lg p-4 bg-gray-50 max-h-[300px] overflow-y-auto"></div>
            <div class="mt-2 text-sm text-gray-600">Total Bobot: <span id="totalBobot">0%</span></div>
            <p class="italic text-blue-600 text-sm mt-1">Gunakan tombol bagi rata jika ingin mendistribusikan secara merata</p>
        </div>
...
<script>
    ...
    cplSelect.addEventListener('change', function() {
        ...
        fetch("{{ route('admin.capaianpembelajaranmatakuliah.getMKByCPL') }}", {
            ...
            body: JSON.stringify({ id_cpls: [idCPL] })
        })
        .then(res => res.json())
        .then(data => {
            ...
            data.forEach((mk, index) => {
                ...
                mkItem.className = 'mb-3 flex items-center justify-between bg-white p-3 border rounded hover:bg-blue-50';
