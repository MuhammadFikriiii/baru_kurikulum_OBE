                        ...
                        <tbody>
                            @foreach ($prodis as $index => $prodi)
                                <tr class="{{ $index % 2 == 0 ? 'bg-gray-50' : 'bg-white' }} hover:bg-gray-100">
                                    ...
                                    <td class="px-6 py-4 text-center text-sm border">{{ $prodi->jurusan?->nama_jurusan ?? '-' }}</td>
                                    ...
                                    <td class="px-6 py-4 text-center text-sm border">{{ $prodi->jenjang_pendidikan ?? '-' }}</td>
                                    <td class="px-6 py-4 text-center text-sm border">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                            {{ $prodi->peringkat_akreditasi == 'A' ? 'bg-green-100 text-green-800' : 
                                               ...
                                               ($prodi->peringkat_akreditasi == 'C' ? 'bg-yellow-100 text-yellow-800' : 
                                    ...
                                    <td class="px-6 py-4 text-center border">
                                        <div class="flex justify-center space-x-2">
                                            <a href="{{ route('admin.prodi.detail', $prodi->kode_prodi) }}" class="bg-gray-600 hover:bg-gray-700 text-white p-2 rounded-md" title="Detail">
                                            ...
                                            <a href="{{ route('admin.prodi.edit', $prodi->kode_prodi) }}" class="bg-blue-600 hover:bg-blue-800 text-white p-2 rounded-md" title="Edit">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="currentColor" viewBox="0 0 20 20">
                                                    <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                                </svg>
                                            </a>
                                            <form action="{{ route('admin.prodi.destroy', $prodi->kode_prodi) }}" method="POST" onsubmit="return confirm('Hapus prodi ini?')">
                                                @csrf @method('DELETE')
                                                <button type="submit" class="bg-red-600 hover:bg-red-800 text-white p-2 rounded-md" title="Hapus">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="currentColor" viewBox="0 0 20 20">
