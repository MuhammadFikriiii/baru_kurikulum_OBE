...
<div class="mr-20 ml-20">
    ...
    @if(session('success'))
        <div id="alert" class="bg-green-500 text-white px-4 py-2 rounded-md mb-4 text-center relative">
            ...
            <button onclick="document.getElementById('alert').style.display='none'" class="absolute top-1 right-3 text-white font-bold text-lg">
                &times;
            </button>
        </div>
    @endif
    ...
    <div class="flex justify-between mb-4">
        <a href="{{ route('tim.penilaian.create') }}" class="bg-green-600 h-10 font-bold text-white px-5 inline-flex items-center py-2 rounded-md hover:bg-green-800">
            Tambah
        </a>
        <div class="ml-auto">
            <input type="text" id="search" placeholder="Search..." class="border border-black px-3 py-2 rounded-md">
        </div>
    </div>
                ...
                <tr class="{{ $index % 2 == 0 ? 'bg-gray-100' : 'bg-white' }} hover:bg-gray-200 border-b">
                    ...
                    <td class="py-3 px-4 flex justify-center items-center space-x-2">
                        ...
                        <form action="#" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                            @csrf
                            @method('DELETE')
                            <button class="bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-600">🗑️</button>
                        </form>
                    </td>
                </tr>
                @endforeach
