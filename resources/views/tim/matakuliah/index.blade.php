...
<div class="flex justify-between mb-4">
    <div class="space-x-2">
        <a href="{{ route('tim.matakuliah.create') }}" class="bg-green-600 inline-flex text-white font-bold px-4 py-2 rounded-md hover:bg-green-800">
            Tambah
        </a>
    </div>
    ...
    <tbody>
        ...
        <tr class="{{ $index % 2 == 0 ? 'bg-gray-100' : 'bg-white' }} hover:bg-gray-200 border-b">
            ...
            <td class="py-2 px-3 flex justify-center items-center space-x-2">
                <a href="{{ route('tim.matakuliah.detail', $mata_kuliah->kode_mk) }}" class="bg-gray-600 font-bold text-white px-5 py-2 rounded-md hover:bg-gray-700">🛈</a>
                <a href="{{ route('tim.matakuliah.edit',$mata_kuliah->kode_mk) }}" class="bg-blue-600 text-white font-bold px-5 py-2 rounded-md hover:bg-blue-800">✏️</a>
                <form action="{{ route('tim.matakuliah.destroy',$mata_kuliah->kode_mk) }}" method="POST">
                    @csrf @method('DELETE')
                <button type="submit" class="bg-red-500 text-white px-5 py-2 rounded-md hover:bg-red-600" onclick="return confirm('Hapus MK ini?')">
                    🗑️
                </button>
                </form>
            </td>
